...
import io.mosip.tuvali.transfer.Util.Companion.byteArrayToInt
...
class TransferReport  {
  ...
  constructor(bytes: ByteArray){
    ...
    this.totalPages = byteArrayToInt(byteArrayOf(bytes[1], bytes[2]), TwoBytes)
    ...
    this.missingSequences = sequenceNumberByteArray.chunked(2)
      ...
      { acc, twoBytes ->
        if (twoBytes.size != 2) {
          ...
          acc + byteArrayToInt(twoBytes.toByteArray(),TwoBytes)
        }
      }
  }
  ...
  */
  fun toByteArray(): ByteArray {
    val missingSeqBytes = missingSequences?.fold(byteArrayOf()) { acc, sNo -> acc + Util.intToByteArray(sNo, TwoBytes) }
    val metadata = byteArrayOf(type.ordinal.toByte()) + Util.intToByteArray(totalPages, TwoBytes)
