package io.mosip.verifier

import android.content.Context
import android.os.HandlerThread
import android.os.Process.THREAD_PRIORITY_DEFAULT
import android.util.Log
import io.mosip.ble.peripheral.IPeripheralListener
import io.mosip.ble.peripheral.Peripheral
import io.mosip.cryptography.SecretsTranslator
import io.mosip.cryptography.VerifierCryptoBox
import io.mosip.cryptography.VerifierCryptoBoxBuilder
import com.facebook.react.bridge.Callback
import io.mosip.transfer.Semaphore
import io.mosip.transfer.Util
import io.mosip.verifier.transfer.ITransferListener
import io.mosip.verifier.transfer.TransferHandler
import io.mosip.verifier.transfer.message.*
  ...
  fun notifyVerificationStatus(accepted: Boolean) {
    if(accepted) {
      peripheral.sendData(SERVICE_UUID, GattService.VERIFICATION_STATUS_CHAR_UUID,
        byteArrayOf(io.mosip.wallet.transfer.TransferHandler.VerificationStates.ACCEPTED.ordinal.toByte()))
    } else {
      peripheral.sendData(SERVICE_UUID, GattService.VERIFICATION_STATUS_CHAR_UUID,
        byteArrayOf(io.mosip.wallet.transfer.TransferHandler.VerificationStates.REJECTED.ordinal.toByte()))
    }
  }
