...
class TransferHandler {
    ...
    private func handleMessage(msg: imessage) {
        if msg.msgType == .INIT_RESPONSE_TRANSFER {
            ...
            currentState = States.ResponseWriteFailed
            ErrorHandler.sharedInstance.handle(type: .walletException, error: .responseTransferFailure)
        } else {
    ...
    private func handleTransmissionReport(report: Data) {
        ...
        if (r.type == .SUCCESS) {
            ...
            EventEmitter.sharedInstance.emitTransferUpdateEvent(status: EventEmitter.TransferUpdateStatus.SUCCESS)
...
extension TransferHandler: PeripheralCommunicatorProtocol {
    ...
    func onVerificationStatusChange(data: Data?) {
        ...
        if let value =  value {
            ...
            if status == 0 {
                EventEmitter.sharedInstance.emitVerificationStatusEvent(status: EventEmitter.VerificationStatus.ACCEPTED)
            } else if status == 1 {
                EventEmitter.sharedInstance.emitVerificationStatusEvent(status: EventEmitter.VerificationStatus.REJECTED)
            }
            destroyConnection?()
        }
    }
