...
class TransferReport  {
    ...
    init(bytes: Data) {
        if bytes.count >= 3 {
            ...
            totalPages = Utils.networkOrderedByteArrayToInt(num: bytes.subdata(in: (bytes.startIndex+1..<bytes.startIndex+3)))
            ...
            if missingChunksData.count > 0 {
                ...
                while missingChunksData.count >= 2 {
                    ...
                    missingSequences?.append(Utils.networkOrderedByteArrayToInt(num: missingChunkSlice))
